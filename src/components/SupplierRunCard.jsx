          ...
          {driverFocused ? (
            <div className="space-y-3">
              {activePickupItem ? (() => {
                ...
                return (
                  <div className={`rounded-2xl border p-4 ${materialUseClasses.card}`}>
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="text-xs font-black uppercase tracking-[0.18em] text-blue-700">
                          Item {activePickupItemIndex + 1} of {items.length}
                        </p>

                        <div className="mt-2 flex flex-wrap items-center gap-2">
                          <span
                            className={`inline-flex rounded-full border px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.12em] ${materialUseClasses.badge}`}
                          >
                            {getMaterialUseLabel(item.materialUse)}
                          </span>

                          {item.quantity ? (
                            <span className="rounded-full bg-white px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.12em] text-blue-700">
                              QTY: {item.quantity}
                            </span>
                          ) : null}
                        </div>
                        ...
                        {item.easilyDamaged ? (
                          <span className="mt-2 inline-flex rounded-full border border-amber-200 bg-amber-50 px-3 py-1.5 text-[11px] font-black uppercase tracking-[0.08em] text-amber-800">
                        ...
                        {item.internalReference ? (
                          <p className="mt-2 text-sm font-bold text-slate-500">
                            SKU / Item # / SO#: {item.internalReference}
                          </p>
                        ) : null}
                    ...
                    {photoReminder ? (
                      <div className="mt-3 rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-sm font-black text-red-700">
                        {photoReminder}
                      </div>
                    ) : null}
                ...
                <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-5 text-center">
                  ...
                  <p className="mt-1 text-sm font-bold text-emerald-700">
                    Finish the vendor stop when every PO at this supplier is complete.
                  </p>
                </div>
              )}

              {upcomingPickupItems.length > 0 ? (
                <div className="rounded-2xl border border-slate-200 bg-white p-3">
                  <p className="text-xs font-black uppercase tracking-[0.16em] text-slate-500">
                    Up Next
                  </p>
                  <div className="mt-2 space-y-2">
                    {upcomingPickupItems.map((item) => (
                      <div
                        key={item.id}
              ...
              {completedPickupItems.length > 0 ? (
                <div className="rounded-2xl border border-emerald-100 bg-emerald-50/70 p-3">
                  <p className="text-xs font-black uppercase tracking-[0.16em] text-emerald-700">
                    Picked Up
                  </p>
                  <div className="mt-2 space-y-2">
                    {completedPickupItems.map((item) => {
                      const pickupPhotos = getPickupPhotoList(item);
                      return (
                        <div
                          key={item.id}
                        ...
                        >
                          ...
                          <span className="shrink-0 rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-black text-emerald-700">
                            {pickupPhotos.length} photo{pickupPhotos.length === 1 ? "" : "s"}
                          </span>
                        </div>
                      );
                    })}
